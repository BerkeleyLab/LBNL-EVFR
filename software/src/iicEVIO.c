/*
 * Xilinx AXI IIC to FMC slot 1 (EVIO)
 */
#include <stdio.h>
#include <stdint.h>
...
{
    if (debugFlags & DEBUGFLAG_IIC_EVIO_REG) {
        printf("%X <= %X\n", reg, value);
    }
...
iicEVIOinit(void)
{
   int gpo = iicEVIOgetGPO();
    int sr;
    if (debugFlags & DEBUGFLAG_IIC_EVIO) {
        printf("iicEVIOinit()\n");
    }
...
int
iicEVIOsend(int address7, int subaddress, const unsigned char *buf, int len)
{
    int i;
...
int
iicEVIOrecv(int address7, int subaddress, unsigned char *buf, int len)
{
